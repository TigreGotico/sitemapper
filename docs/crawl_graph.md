...
bounded BFS starting from *base_url*. It extracts `<a href>` links from each
...
| `same_domain`   | True    | Restrict enqueuing to the same host (treats `www.` as equivalent). |
| `use_sitemap`   | True    | Seed additional URLs from the site's sitemaps. |
| `respect_robots`| True    | Follow `Disallow` rules from robots.txt. |
...
`out_degree` and `in_degree` appear in the `to_dict()` output.

## Asset filtering

Links to non-HTML resources (`.jpg`, `.css`, `.js`, `.pdf`, and similar) are
recorded as edges but never enqueued for crawling.
...
---
[← Discovery](discovery.md) · [Home](../README.md) · [Transport →](transport.md)
